...
object ClientScalaTest {
  ...
  def validateFailure(v: Validation[Error, Boolean]): Option[String] = v match {
    ...
    case scalaz.Failure(HttpError(_, error)) => Some(error)
  ...
  def validateErrorResponseResourceParse(resource: String)(f: (Int, Array[Byte]) => Validation[Error, ServerResponse])(implicit success: String => Unit): Option[String] =
    validateErrorResponseResourceParseWithBoolean(resource)((statusCode, response) => f(statusCode, response) match {
      case scalaz.Success(value) if !value.isSuccess => scalaz.Failure(HttpError(statusCode, s"${HttpResponseStatus.valueOf(statusCode)}"))
  ...
  def onlyTestIfCanConnectToHost(host: String = DEFAULT_HOST, port: Int = DEFAULT_PORT, secure: Boolean = DEFAULT_SECURE)(success: (String, Int, Boolean) => Unit): Unit = {
    ...
    if (result)
      success(host, port, secure)
  }

  def ignoreIfHostNotUp(success: (String, Int, Boolean) => Unit): Unit =
